...
MI_rep_data=MI_rep_data.iloc[1:5,:]
...
def num_COI_splits(partition,COI_LIST):
    ...
    for COI in COI_LIST:
        ...
        result[COI]=np.count_nonzero(colForCOI.to_numpy())-1 
    return result

def UncertainityOfMembership(partition,COI_LIST):
    #Implements UoM: Sum of p_i*log(p_i)
    michigan_data["current"]=get_current_assignment_toindex(partition)
    result={}
    groupedData=(michigan_data.groupby(["current"]).sum())
    for COI in COI_LIST:
...
def EffectiveSplitsIndex(partition,COI_LIST):
    #Implements effective splits 1/(sum(p_i^2))-1
    michigan_data["current"]=get_current_assignment_toindex(partition)
    result={}
    groupedData=(michigan_data.groupby(["current"]).sum())
    for COI in COI_LIST:
        colForCOI=groupedData[COI]
        result[COI]=(1/(sum([colForCOI[indT]**2 for indT in range(len(colForCOI))])))-1
    return result
...
proposal = partial(
    recom, pop_col=popCol, pop_target=ideal_population, epsilon=0.01, node_repeats=2
)
...
partition_info=pd.DataFrame(columns=["Step",
                                     ...
                                     "Efficiency_gap",
                                     "Dem_Victories",
                                    ]+["COI"+str(i)+"_Splits" for i in range(len(COIs))]+
                                    ["COI"+str(i)+"_UoM" for i in range(len(COIs))]+
                                     ["COI"+str(i)+"_ESI" for i in range(len(COIs))]
...
for part in chain: 
    ...
    if(i<burnInTime and i>0):
        ...
        COI_Splits=num_COI_splits(part,["COI"+str(i) for i in range(len(COIs))])
        COI_EFIs=EffectiveSplitsIndex(part,["COI"+str(i) for i in range(len(COIs))])
        ...
        partition_info.loc[len(partition_info.index)] = [i, 
                            ...
                            ]+[COI_Splits["COI"+str(i)] for i in range(len(COIs)) #Number splits
                            ]+[COI_UMS["COI"+str(i)] for i in range(len(COIs)) #Uncertainity Of Membership
                            ]+[COI_EFIs["COI"+str(i)] for i in range(len(COIs))] #Effective Splits Index
    ...
    if(i%50==0):
        ...
        partition_info.to_csv("Cong2.csv")
